...
class Updatestudentcontact extends Common
{
    public function index()
    {
        ...
        $this->assign('address',$this->check($contact->address));
        $this->assign('parent',$this->check($contact->parent));
        $this->assign('parent_phone',$this->check($contact->parent_phone));
    ...
    public function updateselfinfo(){  // 更改个人联系信息功能，电话、邮箱居住地址
        $request = Request::instance();
        // 插入contact表格
        $user = usercontact::get($this->auth->getUserid());
        // 检查所有字段都为空时，用户没有输入任何信息
        if(!($request->param("newphone") or $request->param("newemail") or $request->param("newparent") or
        $request->param("newaddress") or $request->param("newparentphone"))){
        ...
        $parent = $this->update("newparent", $user->parent, $request);
        $parent_phone = $this->update("newparentphone", $user->parent_phone, $request);
